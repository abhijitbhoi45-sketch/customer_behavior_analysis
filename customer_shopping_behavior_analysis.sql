...
select
	gender, sum(purchase_amount) revenue
from
	customer
group by
	gender;
...
select
	item_purchased , round(avg(review_rating),2) avg_product_rating
from 
	customer
group by
	item_purchased
...
select
	subscription_status,
    count(customer_id) repeate_buyers
from
	customer
where 
